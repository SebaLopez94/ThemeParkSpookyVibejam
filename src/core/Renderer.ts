...
export class GameRenderer {
  ...
  private patchHeightFogMaterial(material: THREE.Material): void {
    ...
    material.onBeforeCompile = (shader, renderer) => {
      ...
      shader.uniforms.uNoiseOctaves = { value: this.mobile ? 3 : 5 };
      shader.uniforms.uWindDir = { value: new THREE.Vector2(0.0, 1.0) };
      shader.uniforms.uWindSpeed = { value: this.mobile ? 0.16 : 0.23 };
      shader.uniforms.uVerticalBillow = { value: this.mobile ? 0.008 : 0.014 };
        ...
        if (vWorldPos.y < uFogTop) {
          ...
          if (uFogRadius > 0.0 && distXZ <= uFogRadius) {
            ...
            if (heightAtten > 0.0) {
              ...
              float effectiveStrength = uNoiseStrength * 0.025;
              float noiseMod = mix(1.0 - effectiveStrength, 1.0 + effectiveStrength, noiseMix);
              float radiusFactor = 1.0 - smoothstep(uFogRadius * 0.74, uFogRadius, distXZ);
              float distanceBoost = mix(1.0, 1.24, smoothstep(uFogRadius * 0.18, uFogRadius * 0.82, distXZ));
              float groundBoost = mix(1.05, 1.42, pow(normalized, 1.25));
              float fogFactor = clamp(heightAtten * noiseMod * uFogOpacity * radiusFactor * distanceBoost * groundBoost, 0.0, 1.0);
              vec3 fogTint = mix(uFogColor * 0.94, uFogColor * 1.03, smoothstep(0.12, 0.88, noiseMix));
    ...
    material.customProgramCacheKey = () => `${originalCustomProgramCacheKey()}|height-fog-v3|${material.uuid}`;
  ...
  private updateHeightFog(): void {
    this.heightFogMaterials.forEach(material => {
      ...
      if (!handle?.uniforms.uTime) return;
      handle.uniforms.uTime.value = this.fogTime;
    });
  }
  ...
  public render(scene: THREE.Scene, camera: THREE.Camera): void {
    const patchEvery = this.mobile ? 240 : 120;
    if (this.fogPatchFrame++ % patchEvery === 0) {
      this.patchSceneHeightFog(scene);
    }
    this.fogTime += this.mobile ? 0.0048 : 0.0072;
    const uniformEvery = this.mobile ? 3 : 2;
    if (this.fogUniformFrame++ % uniformEvery === 0) {
